#pragma once

/* Color VGA (primer plano: nibble bajo, fondo: nibble alto) */

#define C_FG_BLACK              (0x0)
#define C_FG_BLUE               (0x1)
#define C_FG_GREEN              (0x2)
#define C_FG_CYAN               (0x3)
#define C_FG_RED                (0x4)
#define C_FG_MAGENTA            (0x5)
#define C_FG_BROWN              (0x6)
#define C_FG_LIGHT_GREY         (0x7)
#define C_FG_DARK_GREY          (0x8)
#define C_FG_LIGHT_BLUE         (0x9)
#define C_FG_LIGHT_GREEN        (0xA)
#define C_FG_LIGHT_CYAN         (0xB)
#define C_FG_LIGHT_RED          (0xC)
#define C_FG_LIGHT_MAGENTA      (0xD)
#define C_FG_LIGHT_BROWN        (0xE)
#define C_FG_WHITE              (0xF)

#define C_BG_BLACK              (0x0 << 4)
#define C_BG_BLUE               (0x1 << 4)
#define C_BG_GREEN              (0x2 << 4)
#define C_BG_CYAN               (0x3 << 4)
#define C_BG_RED                (0x4 << 4)
#define C_BG_MAGENTA            (0x5 << 4)
#define C_BG_BROWN              (0x6 << 4)
#define C_BG_LIGHT_GREY         (0x7 << 4)

#define C_BLINK                 (0x8 << 4)

#define C_BG                    C_BG_BLACK
#define C_PLAYER_A              C_BG_RED  | C_FG_WHITE
#define C_PLAYER_B              C_BG_BLUE | C_FG_WHITE
#define C_PLAYER_A2             C_BG_BLACK | C_FG_RED
#define C_PLAYER_B2             C_BG_BLACK | C_FG_BLUE
#define C_TABLERO               C_BG_LIGHT_GREY | C_FG_LIGHT_GREY
#define C_CRYSTAL_1             C_BG_GREEN | C_FG_LIGHT_GREEN
#define C_CRYSTAL_2             C_BG_BROWN | C_FG_LIGHT_BROWN
#define C_CRYSTAL_3             C_BG_MAGENTA | C_FG_LIGHT_MAGENTA

/* Macro para obtener el color en tiempo de ejecución */
#define C_CRYSTAL(n) \
    (n == 0) ? C_TABLERO   : \
    (n == 1) ? C_CRYSTAL_1 : \
    (n == 2) ? C_CRYSTAL_2 : \
    C_CRYSTAL_3

